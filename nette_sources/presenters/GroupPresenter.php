...
final class GroupPresenter extends BasePresenter
{
	...
	*/
	...
	protected function createComponentUpdateform()
	{
		...
		$form->addFile('group_avatar', _('Upload group image:'))->setOption('description', NHtml::el('img')->src(NEnvironment::getHttpRequest()->uri->scriptPath . 'images/help.png')->class('help-icon')->title(_('Avatars are small images that will be visible with your name. Here you can upload an avatar for your group (upload min. 80x100px, max. 2048x2048px). In the next step you can crop it.'))->id("help-name"))->addCondition(NForm::FILLED)->addRule(NForm::MIME_TYPE, _('Image must be in JPEG or PNG format.'), 'image/jpeg,image/png')->addRule(NForm::MAX_FILE_SIZE, _('Maximum image size is 512kB'), 512 * 1024);
	...
	public function updateformSubmitted(NAppForm $form)
	{
		...
		if ($values['group_avatar']->getTemporaryFile() != "") {
			...
			if ($size[0]>2048 || $size[1]>2048) {
				$this->flashMessage(_("The image is too big! Max. size is for upload is 2048x2048"),'error');
			} else {
